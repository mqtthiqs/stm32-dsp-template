...
struct Leds
{
  ...
  void set(LED led, bool status) {
    ...
    if (led == LED_1) HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, s);
    if (led == LED_2) HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, s);
    if (led == LED_3) HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, s);
    if (led == LED_4) HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15, s);
  }

  void toggle(LED led) {
    if (led == LED_1) HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_13);
    if (led == LED_2) HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
